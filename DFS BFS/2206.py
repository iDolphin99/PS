...
def BFS():
    ...
    visited = [[[0, 0] for _ in range(M)] for _ in range(N)] # 1: 안부수는, 0: 부순
    visited[0][0][1] = 1
    ...
    while deq:
        ...
        for dx, dy in directions:
            ...
            if 0 <= newX < N and 0 <= newY < M:
                # 벽이 없다면?
                if graph[newX][newY] == 0 and visited[newX][newY][1] == 0: 
                    # 이전에 벽뚫 한 적이 없다면? 
                    if break_cnt == 1 :
                        visited[newX][newY][break_cnt] = visited[x][y][break_cnt] + 1
                        deq.append((newX, newY, break_cnt))
                    # 이전에 벽뚤한 적이 있다면?
                    if break_cnt == 0 and visited[newX][newY][break_cnt] == 0:
                        visited[newX][newY][break_cnt] = visited[x][y][break_cnt] + 1 
                        deq.append((newX, newY, break_cnt))
